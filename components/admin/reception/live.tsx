...
import {
  emit_admin_reception_live_action,
  emit_admin_reception_live_message,
  emit_admin_reception_live_support_action,
} from './live_listeners'
...
export default function AdminReceptionLive(props: admin_reception_live_props) {
  ...
  const on_support_action = useCallback(
    (action: chat_action_realtime_payload) => {
      emit_admin_reception_live_support_action(action)
    },
    [],
  )

  const on_message = useCallback((message: realtime_archived_message) => {
    return emit_admin_reception_live_message(message) ?? noop_append
  }, [])

  const on_action = useCallback(
    (action: chat_action_realtime_payload, inserted_index: number) => {
      return emit_admin_reception_live_action(action, inserted_index) ?? noop_append
    },
    [],
  )
